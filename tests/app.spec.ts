...
test('renders a semantic three-step How it works section after the product', async ({ page }) => {
  const how = page.locator('main > section#how');
  await expect(how.getByRole('heading', { level: 2, name: 'How it works' })).toBeVisible();
  await expect(how.locator('ol > li')).toHaveCount(3);
  ...
  expect(await page.evaluate(() => {
    ...
    return Boolean(planner && section && (planner.compareDocumentPosition(section) & Node.DOCUMENT_POSITION_FOLLOWING));
  })).toBe(true);
});
